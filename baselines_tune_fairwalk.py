...
def kmeans_fairwalk(all_features, fold_gen, all_attributes, num_walks, walk_lens, learning_rates, epochs, args, attr_id=0):
    import logging
    logging.basicConfig(format='%(asctime)s %(message)s', level=logging.INFO)
    results = []
    for num_walk in num_walks:
        for walk_len in walk_lens:
            for lr in learning_rates:
                for epoch in epochs:
                    logging.info(f'fairwalk: num_walk={num_walk}, walk_len={walk_len}, learning_rate={lr}, epoch={epoch}')
...
def main():
    ...
    num_walks = [20]
    walk_lens = [80]
    learning_rates = [0.01, 0.025, 0.1, 0.2, 0.5, 1.0]
    epochs = [1, 5, 10]
    logging.info('get_data')
    rlist = kmeans_fairwalk(features, fold_names, attributes, num_walks, walk_lens, learning_rates, epochs, args)
    logging.info('embed_fairwalk')


    results['fairwalk'] = rlist

    if not os.path.exists(f'./results/{args.dataset}'):
        os.mkdir(f'./results/{args.dataset}')

    with open(f'./results/{args.dataset}/baseline_results_tune_fairwalk_lr_ep.json', 'w') as fp:
